...
DELETE
FROM REACAO
WHERE CODIGO IN(SELECT REACAO.CODIGO FROM REACAO JOIN POST ON REACAO.COD_POST = POST.CODIGO 
JOIN GRUPO ON POST.CODIGOGRUPO= GRUPO.CODIGO
...
WHERE CODIGO IN(SELECT COMPARTILHAMENTO.CODIGO FROM COMPARTILHAMENTO
JOIN POST ON COMPARTILHAMENTO.COD_POST = POST.CODIGO 
JOIN GRUPO ON POST.CODIGOGRUPO= GRUPO.CODIGO
WHERE UPPER(POST.CLASSIFICACAO) = 'ODIO'
AND CODIGOGRUPO = (SELECT CODIGO FROM GRUPO WHERE NOMEGRUPO = 'IFRS-Campus Rio Grande')
AND DATAPOST = (SELECT MAX(DATAPOST) FROM POST, GRUPO WHERE POST.CODIGOGRUPO=GRUPO.CODIGO AND GRUPO.NOMEGRUPO='IFRS-Campus Rio Grande'));
...
DELETE 
FROM POST WHERE CODIGO IN(SELECT POST.CODIGO FROM POST JOIN GRUPO ON POST.CODIGOGRUPO= GRUPO.CODIGO
WHERE UPPER(POST.CLASSIFICACAO) = 'ODIO'
AND CODIGOGRUPO = (SELECT CODIGO FROM GRUPO WHERE NOMEGRUPO = 'IFRS-Campus Rio Grande')
AND DATAPOST = (SELECT MAX(DATAPOST) FROM POST, GRUPO WHERE POST.CODIGOGRUPO=GRUPO.CODIGO AND GRUPO.NOMEGRUPO='IFRS-Campus Rio Grande'));
